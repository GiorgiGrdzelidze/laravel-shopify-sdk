...
{
    protected $signature = 'shopify:sync:fulfillments
                            {--store= : Specific store domain to sync}
                            {--dry-run : Preview without syncing}';

    protected $description = 'Sync fulfillments from Shopify';

    public function handle(FulfillmentSyncer $syncer): int
    {
        ...
        foreach ($stores as $store) {
            $this->info("Syncing fulfillments for {$store->shop_domain}...");
            ...
            if (empty($syncRun->errors)) {
                ...
                $this->info("✓ Synced {$counts['total']} fulfillments ({$counts['created']} created, {$counts['updated']} updated)");
            } else {
