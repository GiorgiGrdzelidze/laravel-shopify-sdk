...
class SyncMetafieldsCommand extends Command
{
    protected $signature = 'shopify:sync:metafields
                            {--store= : Specific store domain to sync}
                            {--dry-run : Preview without syncing}';

    protected $description = 'Sync metafields from Shopify';

    public function handle(MetafieldSyncer $syncer): int
    {
        ...
        foreach ($stores as $store) {
            $this->info("Syncing metafields for {$store->shop_domain}...");
            ...
            if (empty($syncRun->errors)) {
                ...
                $this->info("✓ Synced {$counts['total']} metafields ({$counts['created']} created, {$counts['updated']} updated)");
            } else {
